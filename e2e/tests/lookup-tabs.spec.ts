...
test.describe('Lookup Tabs', () => {
  ...
  test.skip('should display "Message UUID" tab and "Session ID" tab on main page', async ({ page }) => {
    // Assert - both tabs should be visible on initial load
    // TODO: Activate when DLD-471 is implemented
    await expect(page.getByRole('tab', { name: 'Message UUID' })).toBeVisible();
    await expect(page.getByRole('tab', { name: 'Session ID' })).toBeVisible();
  });

  test.skip('should have "Message UUID" tab active by default', async ({ page }) => {
  ...
  test.skip('should display session-id-input when "Session ID" tab is clicked', async ({ page }) => {
  ...
  test.skip('should complete session ID lookup flow from "Session ID" tab', async ({ page }) => {
